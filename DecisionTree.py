...
def getBestSplit(rows, mostCommonWords):
	...
	for columnIndex in range(0, nbOfColumns):
		...
		# IG: slide 27 step 3
		p1 = float(len(set1)) / float(len(rows))
		p2 = float(len(set2)) / float(len(rows))
		ig = H - (p1 * getEntropy(set1) +  (p2 * getEntropy(set2)))
...
def buildTree(rows, mostCommonWords, depth):
	...
	# If IG is 0, meaning the split was one sided, then is a leaf. Else, is a node and recursively repeat
	if bestInformationGain == 0.0:
		...
		if leftNode == None or rightNode == None:
			raise ValueError("Empty node, check sufficient input!")
...
def predict(tree, conversation):
	...
	if tree.topicCounts != None:
		...
		branch = tree.leftNode if value == 0 else tree.rightNode
...
def kFoldTesting(input, nbOfFolds):
	...
	for i in range(nbOfFolds):
		...
		print "Number of words: " + str(len(mostCommonWords))
		...
		for conversation in parsedValidation:
			...
			test_y.append(conversationTopic)

			prediction = predict(tree, conversation)

			predictions.append(prediction)

		CountVectorProcessing1.ScikitLearnMethod(test_y, predictions)

def testing(data):
	nbOfRows = len(data)

	training = data[:int(0.8 * nbOfRows)]
	testing = data[int(0.8 * nbOfRows):]

	mostCommonWords = findMostCommonWords(training)
	parsedTraining = parseInput(training, mostCommonWords)
	parsedTesting = parseInput(testing, mostCommonWords)
	...
	for conversation in parsedTesting:
...
def main():
	...
	with open('train_input1.csv','rt') as csvfile:
	    reader = csv.reader(csvfile)
	    for row in reader:
	        data.append(row)

	with open('test_input1.csv','rt') as csvfile:
	    reader = csv.reader(csvfile)
	    for row in reader:
	        testData.append(row)
	...
	######################################################
	# Train with 80 and test with remaining train with 20%
	######################################################
	testing(data)
